...
export default async function Analytics() {
  ...
  const ratingData: RatingDatum[] = brews.map((brew) => ({
    date: brew.tastedAt.toISOString().slice(5, 10),
    rating: brew.rating,
    ratio: brew.brewRatio,
  }));

  const tagCountMap = brewTags.reduce<Record<string, TagCountDatum>>((acc, brewTag) => {
  ...
  const feedbackData: FeedbackDatum[] = ['better', 'same', 'worse'].map((name) => ({
    name,
    value: feedback.filter((entry) => entry.value === name).length,
  }));
